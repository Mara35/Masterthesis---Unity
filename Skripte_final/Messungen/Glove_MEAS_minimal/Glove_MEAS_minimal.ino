...
uint32_t gloveSeq = 0;            // MESS: Sequenzzaehler
...
void sendGloveDataUDP(float tMcp, float tPip,
  ...
  byte buf[48];                                    // MESS: 44 -> 48 Byte
  memcpy(buf, values, 44);
  buf[0] = 0xFF; buf[1] = 0x01; buf[2] = 0xFF; buf[3] = gloveId;
  memcpy(buf + 44, &gloveSeq, 4);                  // MESS: Sequenzzaehler ans Ende
...
// ---------- Loop ----------
void loop() {
  handlePingEcho();   // MESS: RTT-Ping zuerst beantworten
  handleCommands();
