/*
 * Project:    SensinGlove - full hand readout (game firmware)
 * File:       SensinGlove_Game
 * Board:      ESP32-WROOM-32E + CD74HC4067 16-channel analog multiplexer
 *
 * Summary:
 *   Firmware for the sensor glove. Fifteen potentiometer channels (three per finger)
 *   are read through a single ADC pin via the CD74HC4067 multiplexer, converted into
 *   joint angles in degrees, smoothed with an exponential filter and broadcast as a
 *   UDP packet at 20 Hz. The Unity application uses these angles to pose the avatar
 *   hand and to detect grasping.
 *
 *   Only the flexion angles (MCP and PIP) of the five fingers are transmitted; the
 *   abduction channels are sampled but not sent.
 *
 * Calibration:
 ...
 *   S3  -> ESP32 GPIO 22
 *
 * Packet format (48 bytes... 44 bytes here):
 *   [0..2] header 0xFF 0x01 0xFF, [3] gloveId, [4..43] ten floats:
 *   thumb MCP/PIP, index MCP/PIP, middle MCP/PIP, ring MCP/PIP, pinky MCP/PIP
 */
...
void connectWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // reflashing. Broadcast UDP has no acknowledgement or retransmission.
...
// is written over it, which keeps the ten angle floats 4-byte aligned in the packet.
...
// ---------- RTT Ping Echo ----------
// The PC sends an 8-byte ping: FE EE FE EE + uint32 id. It is sent back unchanged
// to the sender so that the PC can measure the round-trip time.
void handlePingEcho() {
